...
import {
  User, Shield, CreditCard, Wallet, FileText, CheckCircle2,
  ChevronRight, ChevronLeft, Plus, Trash2, ArrowLeft,
  Building, Phone, Mail, MapPin, Calendar, Hash, Eye, EyeOff, Check, AlertCircle
} from 'lucide-react';
        ...
        {STEPS.map((step) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-2 z-10">
              ...
              <span className={`text-[9px] font-bold uppercase tracking-widest hidden lg:block ${active ? 'text-teal-600' : done ? 'text-teal-400' : 'text-gray-300'}`}>
                {step.label}
              </span>
              <span className={`text-[9px] font-bold uppercase tracking-widest lg:hidden ${active ? 'text-teal-600' : done ? 'text-teal-400' : 'text-gray-300'}`}>
                {step.short}
              </span>
            </div>
          );
...
const Step1 = ({ data, setData, errors }) => (
  <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      <Input label="First Name" required placeholder="e.g. Ravi" value={data.firstName} onChange={e => setData({...data, firstName: e.target.value})} error={errors.firstName} />
      <Input label="Middle Name" placeholder="e.g. Kumar (optional)" value={data.middleName} onChange={e => setData({...data, middleName: e.target.value})} />
      <Input label="Last Name" required placeholder="e.g. Sharma" value={data.lastName} onChange={e => setData({...data, lastName: e.target.value})} error={errors.lastName} />
      ...
      <Input label="Designation" required placeholder="e.g. Senior Developer" value={data.designation} onChange={e => setData({...data, designation: e.target.value})} error={errors.designation} />
    ...
    <div className="space-y-6">
        ...
        {data.pfEnabled && (
          <div className="bg-teal-50/50 border border-teal-100 rounded-2xl p-5 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Input label="Employee Share %" type="number" placeholder="12" value={data.pfEmployee ?? 12} onChange={e => setData({...data, pfEmployee: e.target.value})} />
              </div>
              <div>
                <Input label="Employer PF %" type="number" placeholder="12" value={data.pfEmployer ?? 12} onChange={e => setData({...data, pfEmployer: e.target.value})} />
              </div>
              <div>
                <Input label="Pension %" type="number" placeholder="8.33" value={data.pfPension ?? 8.33} onChange={e => setData({...data, pfPension: e.target.value})} />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3 pt-2 border-t border-teal-100">
              {[
                { label: 'Emp. PF Deduction', val: `₹${pfCalc.empDeduction}/mo` },
                { label: 'Employer Contribution', val: `₹${pfCalc.empContrib}/mo` },
                { label: 'Pension Contribution', val: `₹${pfCalc.pension}/mo` },
              ].map(item => (
                <div key={item.label} className="bg-white rounded-xl p-3 border border-teal-100 text-center">
                  <p className="text-[8px] font-bold uppercase tracking-widest text-teal-400">{item.label}</p>
                  <p className="text-sm font-bold text-[#042f2e] mt-1">{item.val}</p>
                </div>
              ))}
            </div>
            <p className="text-[9px] text-teal-600/70 font-medium">Preview based on ₹25,000 basic. Final values calculated from salary step.</p>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Other Statutory Deductions</p>
        ...
        <div className="space-y-2">
          ...
          {(!data.otherDeductions || data.otherDeductions.length === 0) && (
            <p className="text-xs text-gray-300 font-medium py-2">No custom deductions added. Click "+ Add Row" to begin.</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Input label="Father's Name" required placeholder="e.g. Suresh Sharma" value={data.fatherName} onChange={e => setData({...data, fatherName: e.target.value})} error={errors.fatherName} />
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          {data.ifsc && !ifscValid && <p className="text-[10px] text-rose-500 font-bold flex items-center gap-1"><AlertCircle size={10}/>Invalid IFSC format (e.g. SBIN0001234)</p>}
...
const Step5 = ({ data, setData }) => {
  ...
  const otherAllowancesSum = (data.otherAllowances || []).reduce((s, a) => s + (parseFloat(a.amount) || 0), 0);
  const carPerq = parseFloat(data.carPerquisite) || 0;
  const otherPerqSum = (data.otherPerquisites || []).reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
  const calculatedCTC = basic + hra + travel + daily + otherAllowancesSum + carPerq + otherPerqSum;
  const diff = ctc - calculatedCTC;

  const addAllowance = () => setData({ ...data, otherAllowances: [...(data.otherAllowances || []), { label: '', amount: '' }] });
  const removeAllowance = (i) => setData({ ...data, otherAllowances: data.otherAllowances.filter((_, idx) => idx !== i) });
  ...
  const addPerq = () => setData({ ...data, otherPerquisites: [...(data.otherPerquisites || []), { label: '', amount: '' }] });
  const removePerq = (i) => setData({ ...data, otherPerquisites: data.otherPerquisites.filter((_, idx) => idx !== i) });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-[#042f2e] rounded-[28px] p-6 text-white relative overflow-hidden">
        ...
        <p className="text-[9px] font-bold uppercase tracking-widest text-teal-400 mb-2">Monthly CTC (Cost to Company)</p>
        ...
        <p className="text-[9px] text-teal-400/70 mt-3">Annual: ₹{(ctc * 12).toLocaleString('en-IN')}</p>
      </div>

      {/* Salary Components */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Salary Components</p>
          {diff !== 0 && (
            <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${diff > 0 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
              {diff > 0 ? `₹${diff.toLocaleString('en-IN')} Unallocated` : `Exceeds CTC by ₹${Math.abs(diff).toLocaleString('en-IN')}`}
            </span>
          )}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Basic Salary" type="number" placeholder="e.g. 25000" value={data.basic} onChange={e => setData({...data, basic: e.target.value})} hint="Typically 40–50% of CTC" />
          <Input label="HRA" type="number" placeholder="e.g. 10000" value={data.hra} onChange={e => setData({...data, hra: e.target.value})} hint="Typically 40–50% of Basic" />
          <Input label="Travelling Allowance" type="number" placeholder="e.g. 2000" value={data.travel} onChange={e => setData({...data, travel: e.target.value})} />
          <Input label="Daily Allowance" type="number" placeholder="e.g. 1000" value={data.daily} onChange={e => setData({...data, daily: e.target.value})} />
        </div>

        {/* Dynamic Other Allowances */}
        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Other Allowances</p>
            <div className="flex items-center gap-2">
              {ctc > 0 && diff > 0 && (
                <button
                  type="button"
                  onClick={() => setData({ ...data, otherAllowances: [...(data.otherAllowances || []), { label: 'Special Allowance', amount: String(diff) }] })}
              ...
              <button type="button" onClick={addAllowance} className="flex items-center gap-1.5 text-xs font-bold text-teal-600 hover:text-teal-700 transition-colors"><Plus size={14}/> Add</button>
            </div>
          </div>
          {ctc > 0 && diff > 0 && (data.otherAllowances || []).length === 0 && (
            <div className="flex items-center gap-3 px-4 py-3 bg-amber-50/60 border border-amber-100 rounded-2xl">
              ...
              <p className="text-[11px] font-bold text-amber-700">
                ₹{diff.toLocaleString('en-IN')} of CTC is unallocated. Click "Auto-fill" to add it as a Special Allowance.
              </p>
            </div>
          )}
          {(data.otherAllowances || []).map((a, i) => (
            <div key={i} className="flex items-center gap-3">
              <input type="text" placeholder="Allowance name" value={a.label} onChange={e => updateAllowance(i, 'label', e.target.value)}
              ...
              <input type="number" placeholder="₹ Amount" value={a.amount} onChange={e => updateAllowance(i, 'amount', e.target.value)}
              ...
              <button type="button" onClick={() => removeAllowance(i)} className="p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={14}/></button>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Perquisites */}
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Perquisites</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Car Perquisite" type="number" placeholder="e.g. 1500" value={data.carPerquisite} onChange={e => setData({...data, carPerquisite: e.target.value})} />
        </div>
        <div className="mt-3 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Other Perquisites</p>
            <button type="button" onClick={addPerq} className="flex items-center gap-1.5 text-xs font-bold text-teal-600 hover:text-teal-700 transition-colors"><Plus size={14}/> Add</button>
          </div>
          {(data.otherPerquisites || []).map((p, i) => (
            <div key={i} className="flex items-center gap-3">
              <input type="text" placeholder="Perquisite name" value={p.label} onChange={e => updatePerq(i, 'label', e.target.value)}
                className="flex-1 px-4 py-2.5 rounded-2xl border border-gray-200 text-sm font-medium text-[#042f2e] focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 transition-all placeholder:text-gray-300" />
              <input type="number" placeholder="₹ Amount" value={p.amount} onChange={e => updatePerq(i, 'amount', e.target.value)}
                className="w-32 px-4 py-2.5 rounded-2xl border border-gray-200 text-sm font-medium text-[#042f2e] focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 transition-all placeholder:text-gray-300" />
              <button type="button" onClick={() => removePerq(i)} className="p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={14}/></button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-[24px] border border-gray-100 p-5">
        <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-3">CTC Formula Breakdown</p>
        <div className="space-y-2 text-sm">
          {[
            ['Basic', basic], ['HRA', hra], ['Travel Allowance', travel], ['Daily Allowance', daily],
            ...(data.otherAllowances || []).map(a => [a.label || 'Allowance', parseFloat(a.amount) || 0]),
            ['Car Perquisite', carPerq],
            ...(data.otherPerquisites || []).map(p => [p.label || 'Perquisite', parseFloat(p.amount) || 0]),
          ].map(([label, val], i) => val > 0 && (
            <div key={i} className="flex justify-between text-gray-600 font-medium">
              <span>{label}</span><span>₹{parseFloat(val).toLocaleString('en-IN')}</span>
            </div>
          ))}
          <div className="flex justify-between font-bold text-[#042f2e] border-t border-gray-200 pt-2 mt-2">
            <span>Calculated CTC</span><span>₹{calculatedCTC.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
    </div>
...
const ReviewCard = ({ title, items, onEdit }) => (
  <div className="bg-white rounded-[24px] border border-gray-100 overflow-hidden shadow-sm">
    <div className="flex items-center justify-between px-6 py-4 bg-gray-50/50 border-b border-gray-100">
      ...
      <button type="button" onClick={onEdit} className="text-[10px] font-bold uppercase tracking-widest text-teal-600 hover:text-teal-700 transition-colors">Edit</button>
    </div>
...
const Step6 = ({ basic, statutory, bank, salary, goToStep }) => (
  <div className="space-y-4">
    <SectionTitle title="Review & Confirm" subtitle="Verify all details before creating the employee record" />
    <ReviewCard
      title="Basic Details"
      onEdit={() => goToStep(1)}
      items={[
    ...
    <ReviewCard
      ...
      items={[
        ...
        ['Provident Fund', statutory.pfEnabled ? `Employee ${statutory.pfEmployee ?? 12}% / Employer ${statutory.pfEmployer ?? 12}%` : 'Disabled'],
        ['TDS', statutory.tds ? 'Applicable' : 'Not Applicable'],
        ['Other Deductions', (statutory.otherDeductions || []).length > 0 ? `${statutory.otherDeductions.length} rows` : 'None'],
      ]}
    />
    <ReviewCard
      ...
      onEdit={() => goToStep(5)}
      items={[
        ['Monthly CTC', `₹${parseFloat(salary.ctc || 0).toLocaleString('en-IN')}`],
        ['Basic', `₹${parseFloat(salary.basic || 0).toLocaleString('en-IN')}`],
        ['HRA', `₹${parseFloat(salary.hra || 0).toLocaleString('en-IN')}`],
        ['Travel Allowance', `₹${parseFloat(salary.travel || 0).toLocaleString('en-IN')}`],
        ['Daily Allowance', `₹${parseFloat(salary.daily || 0).toLocaleString('en-IN')}`],
      ]}
    />
    <ReviewCard
      title="Bank Details"
...
const AddEmployee = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    salary:    { ctc:'', basic:'', hra:'', travel:'', daily:'', otherAllowances:[], carPerquisite:'', otherPerquisites:[] },
  });
  ...
  return (
    <AdminLayout title="Add Employee">
      <div className="max-w-4xl mx-auto space-y-6 pb-32">
        ...
        {/* Subtitle */}
        <p className="text-xs text-gray-400 font-medium -mt-4 mb-4 px-2">Complete all 6 steps to create a fully configured employee record.</p>
        ...
        <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm p-8 min-h-[400px] flex flex-col">
          <div className="flex-1">
            ...
            {step === 2 && <Step2 data={formData.statutory} setData={setSection('statutory')} />}
            ...
            {step === 5 && <Step5 data={formData.salary} setData={setSection('salary')} />}
            {step === 6 && <Step6 basic={formData.basic} statutory={formData.statutory} bank={formData.bank} salary={formData.salary} goToStep={(s) => { setStep(s); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />}
          </div>
